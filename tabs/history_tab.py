...
class HistoryTab(QWidget):
    ...
    def populate_history_table(self):
        ...
        for i, item in enumerate(reversed(self.history_data)):
            ...
            play_btn.clicked.connect(
                lambda checked, idx=original_index: self.play_history_item_by_index(idx)
            )
            ...
            self.history_table.setItem(
                row, 2, QTableWidgetItem(item.get("duration", "--:--"))
            )
            self.history_table.setItem(
                row, 3, QTableWidgetItem(item.get("size", "未知"))
            )
            self.history_table.setItem(
                row, 4, QTableWidgetItem(item.get("format", "未知"))
            )
            self.history_table.setItem(
                row, 5, QTableWidgetItem(item.get("resolution", "未知"))
            )
            self.history_table.setItem(
                row, 6, QTableWidgetItem(item.get("uploader", "未知"))
            )
    ...
    def show_context_menu(self, position):
        ...
        merge_action.triggered.connect(
            lambda: self.prepare_subtitle_merge(history_item)
        )
        menu.addAction(merge_action)

        translate_action = QAction("AI 翻译字幕...", self)
        translate_action.triggered.connect(
            lambda: self.prepare_subtitle_translate(history_item)
        )
        ...
        generate_action.triggered.connect(
            lambda: self.prepare_generate_subtitle(history_item)
        )
